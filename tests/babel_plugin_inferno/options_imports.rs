...
mod imports_option {
    ...
    // babel-plugin-inferno orders the imported helpers differently.
    #[test]
    fn should_import_from_a_custom_module_name() {
        assert_js_eq(
            ...
            r#"import { createVNode, createComponentVNode, createTextVNode, normalizeProps, createFragment } from "inferno-compat";
...
createVNode(1, "div");"#,
    ...
    fn should_import_every_used_helper_in_one_declaration() {
        assert_js_eq(
            ...
            r#"import { createVNode, createComponentVNode, createTextVNode, normalizeProps, createFragment } from "inferno";
createVNode(1, "div", null, [
    normalizeProps(createComponentVNode(2, Foo, {
        ...p
    })),
...
mod existing_bindings {
    ...
    // babel-plugin-inferno calls the existing function. swc-plugin-inferno always calls the helpers it imports; hygiene renames other bindings with the same name.
    #[test]
    fn should_import_create_vnode_next_to_a_top_level_create_vnode_function() {
        assert_js_eq(
            &transform_with(
                "{}",
...
const a = <div/>;",
            ...
            r#"import { createVNode } from "inferno";
function createVNode1() {}
const a = createVNode(1, "div");"#,
        );
    }

    // babel-plugin-inferno calls the existing import. swc-plugin-inferno always calls the helpers it imports; hygiene renames other bindings with the same name.
    #[test]
    fn should_import_create_vnode_next_to_create_vnode_imported_from_another_module() {
        assert_js_eq(
            &transform_with(
                "{}",
...
const a = <div/>;"#,
            ...
            r#"import { createVNode } from "inferno";
import { createVNode as createVNode1 } from "other-lib";
const a = createVNode(1, "div");"#,
        );
    }
...
const a = <div/>;"#,
            ...
            r#"import { createVNode as cv, createVNode } from "inferno";
const a = createVNode(1, "div");"#,
        );
    }
...
const b = createVNode(1, "span");"#,
    ...
    // babel-plugin-inferno assigns the required module to a variable and reads the helpers from it.
    #[test]
    ...
    fn should_require_helpers_from_a_custom_module_name_in_a_script() {
        assert_js_eq(
            ...
            "const { createVNode } = require(\"inferno-compat\");\nconst a = createVNode(1, \"div\");",
        );
    }

    // babel-plugin-inferno calls the declared function. swc-plugin-inferno always calls the helpers it imports; hygiene renames other bindings with the same name.
    #[test]
    fn should_rename_declared_helpers_in_a_script() {
        assert_js_eq(
            &compile_as(
                SourceType::Script,
                "{}",
            ...
            "const { createVNode, createComponentVNode } = require(\"inferno\");\nfunction createVNode1() {}\nconst a = createVNode(1, \"div\", null, createComponentVNode(2, Foo), 2);",
        );
    }

    // babel-plugin-inferno assigns the module to a variable, which needs a unique name.
    #[test]
    fn should_require_helpers_next_to_an_inferno_binding_in_a_script() {
        assert_js_eq(
            &compile_as(
                SourceType::Script,
                "{}",
...
mod current_behaviour_questionable {
    ...
    // babel-plugin-inferno calls the local binding. swc-plugin-inferno always calls the helpers it imports; hygiene renames other bindings with the same name.
    #[test]
...
function f() {
    ...
    // babel-plugin-inferno never adds pure annotations.
    #[test]
    ...
    // babel-plugin-inferno ignores unknown options.
    #[test]
